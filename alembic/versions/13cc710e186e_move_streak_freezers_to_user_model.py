...
def upgrade() -> None:
    ...
    op.drop_index('ix_waitlist_emails_id', table_name='waitlist_emails')
    ...
    op.add_column('users', sa.Column('streak_freezers', sa.Integer(), nullable=False))
...
def downgrade() -> None:
    ...
    sa.Column('status', sa.VARCHAR(length=32), server_default=sa.text("'success'::character varying"), autoincrement=False, nullable=False),
    sa.PrimaryKeyConstraint('id', name='waitlist_emails_pkey')
    )
    op.create_index('ix_waitlist_emails_id', 'waitlist_emails', ['id'], unique=False)
    op.create_index('ix_waitlist_emails_email', 'waitlist_emails', ['email'], unique=False)
