...
@router.get("/public/{username}", response_model=dict, summary="Get Public User Profile")
async def get_public_profile(
    ...
    # Get user's habits
    habits = await habits_crud.get_habits_by_user(db, user.id)

    # Get user's public posts count
    post_count = await PostCRUD.get_user_post_count(db, user.id)

    # Generate signed URL for profile image if it exists
    profile_image_url = None
    if user.profile_image_url:
        base_url = get_base_blob_url(user.profile_image_url)
        profile_image_url = file_upload_service.generate_permanent_signed_url(base_url, container=file_upload_service.pfp_container_name)
